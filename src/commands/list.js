...
exports.run = (bot, message, args) => {
    ...
    if (!args[0]) return bot.logger.log("Must provide a type!");
    if (args[0] === "types") {
        const types = [];
        ...
        return message.edit({embed}).catch(bot.logger.error);
    }
    const list = bot.commands.filter(a => a.type === args[0]);

    if (!list) return bot.logger.log(`${args[0]} is not a valid type!`);
